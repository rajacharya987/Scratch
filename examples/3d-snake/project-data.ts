import projectDataMessages from '../../packages/scratch-gui/src/lib/default-project/messages';
import sharedMessages from '../../packages/scratch-gui/src/lib/shared-messages';
import {MessageObject, TranslatorFunction} from '../../packages/scratch-gui/src/gui-config';
...
const makeBlocks = () => {
    ...
    const put = (partial: Record<string, any>): string => {
    ...
    const attachNext = (parent: string | null, child: string) => {
        if (parent) {
    ...
    const cmd = (opcode: string, parent: string | null, extra: Record<string, any> = {}) =>
        attachNext(parent, put(Object.assign({opcode}, extra)));

    const hat = (opcode: string, x: number, y: number, extra: Record<string, any> = {}) =>
        put(Object.assign({opcode, topLevel: true, x, y}, extra));

    const shadowNum = (parent: string, name: string, value: number | string, opcode = 'math_number') => {
        const id = put({
            opcode,
            parent,
            shadow: true,
    ...
    const shadowText = (parent: string, name: string, value: string) => {
        const id = put({
            opcode: 'text',
            parent,
            shadow: true,
    ...
    const shadowColor = (parent: string, name: string, value: string) => {
        const id = put({
    ...
    const shadowMenu = (parent: string, inputName: string, menuOpcode: string, fieldName: string, value: string) => {
        const id = put({
            opcode: menuOpcode,
            parent,
            shadow: true,
    ...
    const reporter = (opcode: string, parent: string, inputName: string, extra: Record<string, any> = {}) => {
        const id = put(Object.assign({opcode, parent, shadow: false}, extra));
        blocks[parent].inputs[inputName] = [2, id];
        return id;
    };

    const varRep = (parent: string, inputName: string, name: string, varId: string) =>
        reporter('data_variable', parent, inputName, {
            fields: {VARIABLE: [name, varId]}
        });

    const setVar = (parent: string | null, name: string, varId: string, value: string | number) => {
        const id = cmd('data_setvariableto', parent, {
            fields: {VARIABLE: [name, varId]}
        });
    ...
    const xyz = (parent: string | null, x: number, y: number, z: number) => {
        const id = cmd('threed_setPosition', parent);
        shadowNum(id, 'X', x);
        shadowNum(id, 'Y', y);
        shadowNum(id, 'Z', z);
        return id;
    ...
    const mesh = (parent: string | null, shape: string) => {
        const id = cmd('threed_setMesh', parent);
        shadowMenu(id, 'MESH', 'threed_menu_mesh', 'mesh', shape);
        return id;
    };

    const color = (parent: string | null, hex: string) => {
        const id = cmd('threed_setMaterialColor', parent);
        shadowColor(id, 'COLOR', hex);
        return id;
    };

    const scaleXYZ = (parent: string | null, x: number, y: number, z: number) => {
        const id = cmd('threed_setScale', parent);
        shadowNum(id, 'X', x);
        shadowNum(id, 'Y', y);
        shadowNum(id, 'Z', z);
        return id;
    ...
    const scale = (parent: string | null, s: number) => scaleXYZ(parent, s, s, s);

    const rot = (parent: string | null, x: number, y: number, z: number) => {
        const id = cmd('threed_setRotation', parent);
        shadowNum(id, 'X', x);
        shadowNum(id, 'Y', y);
        shadowNum(id, 'Z', z);
        return id;
    ...
    const setNum = (parent: string | null, opcode: string, name: string, value: number) => {
        const id = cmd(opcode, parent);
        shadowNum(id, name, value);
        return id;
    };

    const onOff = (parent: string | null, opcode: string, value: string) => {
        const id = cmd(opcode, parent);
    ...
    return {
        blocks, put, cmd, hat, shadowNum, shadowText, shadowMenu, shadowColor,
        reporter, varRep, setVar, xyz, mesh, color, scale, scaleXYZ, rot, setNum, onOff, attachNext
    };
};

const coverVar = (b: ReturnType<typeof makeBlocks>, parent: string, name: string, varName: string, varId: string) => {
    const textShadow = b.blocks[parent].inputs.VALUE[1];
    const rep = b.reporter('data_variable', parent, 'VALUE', {fields: {VARIABLE: [varName, varId]}});
...
const headBlocks = () => {
    const b = makeBlocks();
    const {
        cmd, hat, shadowNum, shadowText, shadowMenu, shadowColor, reporter, varRep,
        setVar, xyz, mesh, color, scale, rot, setNum, onOff
    } = b;

    let p: string | null = hat('event_whenflagclicked', 40, 40);
    ...
    {
        const c = cmd('threed_setFogPreset', p);
        shadowMenu(c, 'PRESET', 'threed_menu_fogPreset', 'fogPreset', 'off');
        p = c;
    }
    {
        const c = cmd('threed_setSkyColor', p);
        shadowColor(c, 'COLOR', '#5AA8E8');
        p = c;
    }
    {
        const c = cmd('threed_setSkyBottom', p);
        shadowColor(c, 'COLOR', '#C8E6A8');
        p = c;
    }
    {
        const c = cmd('threed_setGroundColor', p);
    ...
    {
        const c = cmd('threed_setLightDirection', p);
        shadowNum(c, 'X', 0.2);
        shadowNum(c, 'Y', -1);
        shadowNum(c, 'Z', 0.15);
        p = c;
    ...
    {
        const c = cmd('threed_setLightColor', p);
    ...
    {
        const c = cmd('threed_setCameraPosition', p);
        shadowNum(c, 'X', 0);
        shadowNum(c, 'Y', 420);
        shadowNum(c, 'Z', 16);
        p = c;
    ...
    {
        const c = cmd('threed_setCameraTarget', p);
        shadowNum(c, 'X', 0);
        shadowNum(c, 'Y', 0);
        shadowNum(c, 'Z', 0);
        p = c;
    ...
    {
        const c = cmd('data_insertatlist', p);
    ...
    {
        const c = cmd('control_create_clone_of', p);
    ...
    const forever = cmd('control_forever', p);
    const ifAlive = cmd('control_if', forever);
    b.blocks[forever].inputs.SUBSTACK = [2, ifAlive];
    b.blocks[ifAlive].parent = forever;
    b.blocks[forever].next = null;

    {
        const and = reporter('operator_and', ifAlive, 'CONDITION');
        const eq = reporter('operator_equals', and, 'OPERAND1');
        varRep(eq, 'OPERAND1', 'alive', ID.alive);
        shadowText(eq, 'OPERAND2', '1');
        const eqP = reporter('operator_equals', and, 'OPERAND2');
        varRep(eqP, 'OPERAND1', 'paused', ID.paused);
        shadowText(eqP, 'OPERAND2', '0');
    }

    let s: string | null = cmd('control_wait', ifAlive);
    b.blocks[ifAlive].inputs.SUBSTACK = [2, s];
    {
        const dur = reporter('operator_add', s, 'DURATION');
        shadowNum(dur, 'NUM1', 0.13);
        const div = reporter('operator_divide', dur, 'NUM2');
        shadowNum(div, 'NUM1', 1.1);
        const add = reporter('operator_add', div, 'NUM2');
    ...
    {
        const c = cmd('data_insertatlist', s);
        b.blocks[c].fields.LIST = ['body x', ID.bodyX];
        shadowNum(c, 'INDEX', 1, 'math_integer');
    ...
    {
        const c = cmd('threed_setPosition', s);
        const addX = reporter('operator_add', c, 'X');
        reporter('threed_getX', addX, 'NUM1');
        varRep(addX, 'NUM2', 'dx', ID.dx);
        shadowNum(c, 'Y', 16);
        const addZ = reporter('operator_add', c, 'Z');
        reporter('threed_getZ', addZ, 'NUM1');
        varRep(addZ, 'NUM2', 'dz', ID.dz);
        s = c;
    }

    const face = (parent: string | null, axisVar: string, axisId: string, value: number, yaw: number) => {
        const iff = cmd('control_if', parent);
        const eq = reporter('operator_equals', iff, 'CONDITION');
    ...
    const ifFood = cmd('control_if_else', s);
    {
        const lt = reporter('operator_lt', ifFood, 'CONDITION');
        const dist = reporter('threed_distanceTo', lt, 'OPERAND1');
        shadowMenu(dist, 'SPRITE', 'threed_menu_sprites', 'sprites', 'Food');
        shadowNum(lt, 'OPERAND2', 34);
    }
    let t: string | null = cmd('data_changevariableby', ifFood);
    b.blocks[ifFood].inputs.SUBSTACK = [2, t];
    b.blocks[t].fields.VARIABLE = ['score', ID.score];
    shadowNum(t, 'VALUE', 1);
    {
        const c = cmd('sound_play', t);
        const sh = b.put({
    ...
    {
        const c = cmd('control_create_clone_of', t);
        shadowMenu(c, 'CLONE_OPTION', 'control_create_clone_of_menu', 'CLONE_OPTION', 'Body');
        t = c;
    }
    {
        const c = cmd('event_broadcast', t);
        shadowMenu(c, 'BROADCAST_INPUT', 'event_broadcast_menu', 'BROADCAST_OPTION', 'place food');
        b.blocks[b.blocks[c].inputs.BROADCAST_INPUT[1]].fields.BROADCAST_OPTION = ['place food', ID.placeFood];
        t = c;
    }
    let el: string | null = cmd('data_deleteoflist', ifFood);
    ...
    const ifWall = cmd('control_if', s);
    {
        const or1 = reporter('operator_or', ifWall, 'CONDITION');
        const or2 = reporter('operator_or', or1, 'OPERAND1');
        const or3 = reporter('operator_or', or1, 'OPERAND2');
        const gtX = reporter('operator_gt', or2, 'OPERAND1');
        reporter('threed_getX', gtX, 'OPERAND1');
        shadowNum(gtX, 'OPERAND2', 168);
        const ltX = reporter('operator_lt', or2, 'OPERAND2');
        reporter('threed_getX', ltX, 'OPERAND1');
        shadowNum(ltX, 'OPERAND2', -168);
        const gtZ = reporter('operator_gt', or3, 'OPERAND1');
        reporter('threed_getZ', gtZ, 'OPERAND1');
        shadowNum(gtZ, 'OPERAND2', 168);
        const ltZ = reporter('operator_lt', or3, 'OPERAND2');
        reporter('threed_getZ', ltZ, 'OPERAND1');
        shadowNum(ltZ, 'OPERAND2', -168);
    }
    let w: string | null = setVar(ifWall, 'alive', ID.alive, 0);
    b.blocks[ifWall].inputs.SUBSTACK = [2, w];
    {
        const c = cmd('looks_say', w);
        shadowText(c, 'MESSAGE', 'Game over — hit the wall.');
    }
    s = ifWall;

    const ifBody = cmd('control_if', s);
    {
        const gt = reporter('operator_gt', ifBody, 'CONDITION');
        reporter('data_lengthoflist', gt, 'OPERAND1', {fields: {LIST: ['body x', ID.bodyX]}});
        shadowNum(gt, 'OPERAND2', 4);
    }
    let loopParent: string | null = setVar(ifBody, 'i', ID.i, 3);
    b.blocks[ifBody].inputs.SUBSTACK = [2, loopParent];
    const repeat = cmd('control_repeat', loopParent);
    {
        const len = reporter('data_lengthoflist', repeat, 'TIMES', {fields: {LIST: ['body x', ID.bodyX]}});
        b.blocks[repeat].inputs.TIMES = [2, len];
    }
    const ifHit = cmd('control_if', repeat);
    b.blocks[repeat].inputs.SUBSTACK = [2, ifHit];
    b.blocks[repeat].next = null;
    {
        const and = reporter('operator_and', ifHit, 'CONDITION');
        const eqX = reporter('operator_equals', and, 'OPERAND1');
        reporter('threed_getX', eqX, 'OPERAND1');
        const itemX = reporter('data_itemoflist', eqX, 'OPERAND2', {fields: {LIST: ['body x', ID.bodyX]}});
        varRep(itemX, 'INDEX', 'i', ID.i);
        const eqZ = reporter('operator_equals', and, 'OPERAND2');
        reporter('threed_getZ', eqZ, 'OPERAND1');
        const itemZ = reporter('data_itemoflist', eqZ, 'OPERAND2', {fields: {LIST: ['body z', ID.bodyZ]}});
        varRep(itemZ, 'INDEX', 'i', ID.i);
    }
    const die = setVar(ifHit, 'alive', ID.alive, 0);
    b.blocks[ifHit].inputs.SUBSTACK = [2, die];
    {
        const c = cmd('looks_say', die);
        shadowText(c, 'MESSAGE', 'Game over — you bit yourself.');
    }
    const bump = cmd('data_changevariableby', ifHit);
    b.blocks[ifHit].next = bump;
    b.blocks[bump].fields.VARIABLE = ['i', ID.i];
    shadowNum(bump, 'VALUE', 1);

    const arrow = (key: string, y: number, ndx: number, ndz: number, blockDx: string, avoid: number) => {
        const h = hat('event_whenkeypressed', 40, y, {
            fields: {KEY_OPTION: [key, null]}
        });
        const iff = cmd('control_if', h);
        delete b.blocks[iff].inputs.CONDITION;
        const notOp = reporter('operator_not', iff, 'CONDITION');
        const eq = reporter('operator_equals', notOp, 'OPERAND');
        varRep(eq, 'OPERAND1', blockDx, blockDx === 'dx' ? ID.dx : ID.dz);
        shadowText(eq, 'OPERAND2', String(avoid));
        let q: string | null = setVar(iff, 'ndx', ID.ndx, ndx);
    ...
    const space = hat('event_whenkeypressed', 420, 900, {
        fields: {KEY_OPTION: ['space', null]}
    });
    const ifPause = cmd('control_if_else', space);
    {
        const eq = reporter('operator_equals', ifPause, 'CONDITION');
...
const foodBlocks = () => {
    const b = makeBlocks();
    const {cmd, hat, shadowNum, shadowMenu, mesh, color, scale, xyz, setNum} = b;
    ...
    p = xyz(p, 80, 16, -80);
    const forever = cmd('control_forever', p);
    const spin = cmd('threed_turnAxis', forever);
    b.blocks[forever].inputs.SUBSTACK = [2, spin];
    b.blocks[forever].next = null;
    shadowMenu(spin, 'AXIS', 'threed_menu_axis', 'axis', 'y');
    shadowNum(spin, 'DEGREES', 8);
    const wait = cmd('control_wait', spin);
    shadowNum(wait, 'DURATION', 0.05);

    const recv = hat('event_whenbroadcastreceived', 40, 420, {
        fields: {BROADCAST_OPTION: ['place food', ID.placeFood]}
    });
    const pos = cmd('threed_setPosition', recv);
    const mulX = b.reporter('operator_multiply', pos, 'X');
    const rndX = b.reporter('operator_random', mulX, 'NUM1');
    shadowNum(rndX, 'FROM', -3);
    shadowNum(rndX, 'TO', 3);
    shadowNum(mulX, 'NUM2', 40);
    shadowNum(pos, 'Y', 16);
    const mulZ = b.reporter('operator_multiply', pos, 'Z');
    const rndZ = b.reporter('operator_random', mulZ, 'NUM1');
...
const bodyBlocks = () => {
    const b = makeBlocks();
    const {cmd, hat, shadowNum, reporter, setVar, mesh, color, scale, setNum} = b;

    let p: string | null = hat('event_whenflagclicked', 40, 40);
    p = cmd('looks_hide', p);

    const cloneHat = hat('control_start_as_clone', 40, 200);
    let c: string | null = cmd('looks_show', cloneHat);
    c = mesh(c, 'sphere');
    c = color(c, '#43A047');
    ...
    {
        const textShadow = b.blocks[c].inputs.VALUE[1];
        const len = reporter('data_lengthoflist', c, 'VALUE', {
    ...
    const forever = cmd('control_forever', c);
    const pos = cmd('threed_setPosition', forever);
    b.blocks[forever].inputs.SUBSTACK = [2, pos];
    b.blocks[forever].next = null;
    b.blocks[pos].parent = forever;
    const itemX = reporter('data_itemoflist', pos, 'X', {fields: {LIST: ['body x', ID.bodyX]}});
    reporter('data_variable', itemX, 'INDEX', {fields: {VARIABLE: ['index', ID.index]}});
    shadowNum(pos, 'Y', 16);
    const itemZ = reporter('data_itemoflist', pos, 'Z', {fields: {LIST: ['body z', ID.bodyZ]}});
    reporter('data_variable', itemZ, 'INDEX', {fields: {VARIABLE: ['index', ID.index]}});
    const wait = cmd('control_wait', pos);
    shadowNum(wait, 'DURATION', 0.05);
    return b.blocks;
};

const propBlocks = (shape: string, hex: string, sx: number, sy: number, sz: number, x: number, y: number, z: number) => {
    const b = makeBlocks();
    const {hat, mesh, color, scaleXYZ, xyz, setNum} = b;
    let p: string | null = hat('event_whenflagclicked', 40, 40);
    p = mesh(p, shape);
    p = color(p, hex);
...
const costumePair = (translator: TranslatorFunction) => ([
    {
...
const meowSound = (translator: TranslatorFunction) => ({
...
const projectData = (translateFunction?: TranslatorFunction): object => {
    ...
    const costumes = costumePair(translator);
    const meow = meowSound(translator);
    ...
    return ({
        targets: [
            ...
            sprite('WallN', 4, propBlocks('cube', '#5D4037', 8.2, 0.55, 0.45, 0, 14, -190), {
                costumes, sounds: [meow], x: 0, y: 14, z: -190,
                mesh: 'cube',
            ...
            sprite('WallE', 6, propBlocks('cube', '#4E342E', 0.45, 0.55, 8.2, 190, 14, 0), {
                costumes, sounds: [meow], x: 190, y: 14, z: 0,
                mesh: 'cube',
