...
class RenderedTarget extends Target {
    ...
    constructor (sprite, runtime) {
        ...
        /**
         * Scratch Z coordinate. 0 is the 2D plane; positive Z is toward the default camera.
         * @type {number}
         */
        this.z = 0;

        /**
         * Euler rotation in degrees (X pitch, Y yaw, Z roll). 2D `direction` still drives 2D drawing.
         * @type {number}
         */
        this.rotationX = 0;
        this.rotationY = 0;
        ...
        /**
         * Non-uniform scale. `size` (percent) is multiplied with these for the 3D pass.
         * @type {number}
         */
        this.scaleX = 1;
        this.scaleY = 1;
        ...
        /**
         * Built-in mesh name (cube, sphere, …) or imported model id. Null means 2D costume only.
         * @type {?string}
         */
        this.mesh = null;

        /**
         * Custom 3D mesh geometry {positions, normals, uvs, indices} from imported 3D models.
         * @type {?object}
         */
        this.customMesh = null;

        /**
         * PBR material for the 3D pass.
         * @type {{albedo: number[], metallic: number, roughness: number, emissive: number, opacity: number}}
         */
        this.material3d = {
    ...
    /**
     * Set the full 3D position. 2D blocks keep using setXY; this is the unified path.
     * @param {number} x X
     * @param {number} y Y
     * @param {number} z Z
     * @param {boolean=} force Ignore drag lock
     */
    setXYZ (x, y, z, force) {
        if (this.isStage) return;
        if (this.dragging && !force) return;
    ...
    setZ (z, force) {
        this.setXYZ(this.x, this.y, z, force);
    }

    setRotation3D (rx, ry, rz) {
        if (this.isStage) return;
        this.rotationX = rx;
        this.rotationY = ry;
        this.rotationZ = rz;
    ...
    setMesh (meshName, customMesh) {
        this.mesh = meshName || null;
        if (customMesh !== undefined) {
            this.customMesh = customMesh;
        }
        this.syncDrawable3D();
        if (this.renderer) {
            this.renderer.updateDrawableVisible(this.drawableID, this.visible && !this.mesh);
            if ((this.mesh || this.customMesh) && this.renderer.scene) {
    ...
    /**
     * Push the unified transform to the 3D renderer, if present.
     */
    syncDrawable3D () {
        if (!this.renderer || !this.renderer.updateObject3D || this.isStage) return;
        const sizeScale = this.size / 100;
        this.renderer.updateObject3D(this.drawableID, {
            mesh: this.mesh,
            customMesh: this.customMesh,
            position: [this.x, this.y, this.z],
            rotation: [this.rotationX, this.rotationY, this.rotationZ],
            scale: [this.scaleX * sizeScale, this.scaleY * sizeScale, this.scaleZ * sizeScale],
            visible: this.visible,
            material: this.material3d
        });
    }
    ...
    setCostume (index) {
        ...
        if (costume && costume.customMesh) {
            this.mesh = costume.mesh || costume.name || 'custom';
            this.customMesh = costume.customMesh;
            if (costume.material3d) {
                this.material3d = Object.assign({}, this.material3d, costume.material3d);
            }
            this.syncDrawable3D();
    ...
    makeClone () {
        ...
        newClone.y = this.y;
        newClone.z = this.z;
        newClone.rotationX = this.rotationX;
        newClone.rotationY = this.rotationY;
        newClone.rotationZ = this.rotationZ;
        newClone.scaleX = this.scaleX;
        newClone.scaleY = this.scaleY;
        newClone.scaleZ = this.scaleZ;
        newClone.mesh = this.mesh;
        newClone.material3d = Clone.simple(this.material3d);
    ...
    duplicate () {
        return this.sprite.duplicate().then(newSprite => {
            ...
            newTarget.z = this.z;
            newTarget.rotationX = this.rotationX;
            newTarget.rotationY = this.rotationY;
            newTarget.rotationZ = this.rotationZ;
            newTarget.scaleX = this.scaleX;
            newTarget.scaleY = this.scaleY;
            newTarget.scaleZ = this.scaleZ;
            newTarget.mesh = this.mesh;
            newTarget.material3d = JSON.parse(JSON.stringify(this.material3d));
