...
class VirtualMachine extends EventEmitter {
    ...
    /**
     * @param {string=} optType Optional type that the resulting zip should be outputted in.
     * Defaults to 'blob'.
     * @returns {Promise<Blob|Uint8Array|string|ArrayBuffer>} Generated zip of the project.
     */
    saveProjectSb3 (optType) {
        ...
        return zip.generateAsync({
            type: typeof optType === 'string' ? optType : 'blob',
    ...
    addCostume (md5ext, costumeObject, optTargetId, optVersion) {
        ...
        if (target) {
            return loadCostume(md5ext, costumeObject, this.runtime, optVersion).then(() => {
                ...
                if (costumeObject.customMesh) {
                    target.mesh = costumeObject.mesh || costumeObject.name || 'custom';
                    target.customMesh = costumeObject.customMesh;
                    if (costumeObject.material3d) {
                        target.material3d = Object.assign({}, target.material3d, costumeObject.material3d);
                    }
                    target.syncDrawable3D();
                    if (this.runtime.renderer && this.runtime.renderer.scene) {
                        this.runtime.renderer.scene.enable();
                    }
                }
