...
const serializeTarget = function (target, extensions) {
    ...
    if (obj.isStage) { // Only the stage should have these properties
        ...
        if (Object.prototype.hasOwnProperty.call(target, 'z')) obj.z = target.z;
        if (Object.prototype.hasOwnProperty.call(target, 'rotationX')) {
            obj.rotationX = target.rotationX;
            obj.rotationY = target.rotationY;
            obj.rotationZ = target.rotationZ;
        }
        if (Object.prototype.hasOwnProperty.call(target, 'scaleX')) {
            obj.scaleX = target.scaleX;
            obj.scaleY = target.scaleY;
            obj.scaleZ = target.scaleZ;
        }
        if (target.mesh) obj.mesh = target.mesh;
        if (target.customMesh) {
            obj.customMesh = {
                positions: Array.from(target.customMesh.positions),
                normals: Array.from(target.customMesh.normals),
                uvs: Array.from(target.customMesh.uvs),
                indices: Array.from(target.customMesh.indices)
            };
        }
        if (target.material3d) obj.material3d = target.material3d;
    }

    if (obj.isStage && target.runtime && target.runtime.renderer &&
        target.runtime.renderer.exportScene3D) {
        const scene = target.runtime.renderer.exportScene3D();
        if (scene && scene.enabled) obj.scene3d = scene;
    }
...
const parseScratchObject = function (object, runtime, extensions, zip, assets) {
    ...
    if (Object.prototype.hasOwnProperty.call(object, 'rotationX')) {
        target.rotationX = object.rotationX || 0;
        target.rotationY = object.rotationY || 0;
        target.rotationZ = object.rotationZ || 0;
    }
    if (Object.prototype.hasOwnProperty.call(object, 'scaleX')) {
        target.scaleX = object.scaleX;
        target.scaleY = object.scaleY;
        target.scaleZ = object.scaleZ;
    }
    if (Object.prototype.hasOwnProperty.call(object, 'mesh')) {
        target.mesh = object.mesh;
    }
    if (Object.prototype.hasOwnProperty.call(object, 'customMesh') && object.customMesh) {
        target.customMesh = {
            positions: new Float32Array(object.customMesh.positions),
            normals: new Float32Array(object.customMesh.normals),
            uvs: new Float32Array(object.customMesh.uvs),
            indices: object.customMesh.indices && object.customMesh.indices.length > 65535 ?
                new Uint32Array(object.customMesh.indices) : new Uint16Array(object.customMesh.indices)
        };
    }
    if (Object.prototype.hasOwnProperty.call(object, 'material3d')) {
        target.material3d = Object.assign({}, target.material3d, object.material3d);
    }
    if (target.setMesh && (target.mesh || target.customMesh)) {
        target.setMesh(target.mesh, target.customMesh);
    }
    if ((target.mesh || target.customMesh) && target.runtime && target.runtime.renderer && target.runtime.renderer.scene) {
        target.runtime.renderer.scene.enable();
    }
    if (object.scene3d && target.runtime) {
        if (target.runtime.renderer && target.runtime.renderer.importScene3D) {
            target.runtime.renderer.importScene3D(object.scene3d);
        } else {
            target.runtime._pendingScene3d = object.scene3d;
        }
    }
