...
const SBFileUploaderHOC = function (WrappedComponent) {
    class SBFileUploaderComponent extends React.Component {
        ...
        createFileObjects () {
            ...
            this.inputElement.accept = '.sb,.sb2,.sb3,.sb3e';
        ...
        async onload () {
            if (this.fileReader) {
                ...
                try {
                    let projectBuffer = this.fileReader.result;
                    if (isEncryptedProject(projectBuffer)) {
                        let pass = '';
                        if (isPasswordProtected(projectBuffer)) {
                            pass = prompt('This project is password protected. Enter password:') || '';
                        }
                        projectBuffer = await decryptProject(projectBuffer, pass);
                    }
                    await this.props.vm.loadProject(projectBuffer);
                    if (filename) {
