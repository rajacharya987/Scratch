...
class Blocks extends React.Component {
    ...
    attachVM () {
        ...
        // EXTENSION_ADDED is missed when the workspace mounts after a project
        // (or HMR remount) already loaded an extension. Define those blocks
        // now so workspace XML can instantiate them.
        const loadedExtensions = this.props.vm.runtime && this.props.vm.runtime._blockInfo;
        if (loadedExtensions && loadedExtensions.length) {
            loadedExtensions.forEach(categoryInfo => {
                this.handleExtensionAdded(categoryInfo);
            });
        }
        // Re-run getInfo so newly added extension blocks (fog, etc.) appear
        // in the toolbox after a workspace remount or live reload.
    ...
    glowWorkspaceStack (id, isGlowing) {
        if (!id) return;
        try {
            this.ScratchBlocks.glowStack(id, isGlowing);
        } catch {
            // The VM glows stacks by block id. scratch-blocks throws if that id
            // is not in this Blockly workspace — another sprite is being
            // edited, workspace XML did not instantiate the block, or the
            // stack lives only in the VM.
        }
    }
    onScriptGlowOn (data) {
        this.glowWorkspaceStack(data.id, true);
    }
    onScriptGlowOff (data) {
        this.glowWorkspaceStack(data.id, false);
    }
    onBlockGlowOn (/* data */) {
    ...
    onVisualReport (data) {
        try {
            ...
            // Same as glow: the reporter may not be in this workspace.
        }
    }
    getToolboxXML () {
