...
class MenuBar extends React.Component {
    ...
    componentDidMount () {
        ...
        if (hashMatch) {
            getSharedProject(hashMatch[1]).then(record => {
                if (record) {
                    this.setState({isShared: true, sharedId: hashMatch[1]});
                }
            }).catch(() => {});
        }
    }
    ...
    handleClickNew () {
        ...
        if (readyToReplaceProject) {
            this.setState({isShared: false, sharedId: null});
    ...
    handleClickShare () {
        if (this.props.isShared || this.state.isShared || this.state.isSharing) {
            return;
        }
        ...
        this.shareStandalone();
    }
    async shareStandalone () {
        if (!this.props.vm || !this.props.vm.saveProjectSb3) return;
        this.setState({isSharing: true});
        this.props.onShowSharingAlert();
        try {
            const id = String(Date.now());
            const saved = await this.props.vm.saveProjectSb3();
            const buffer = saved instanceof Blob ? await saved.arrayBuffer() : saved;
            await putSharedProject(id, {
                title: this.props.projectTitle,
                buffer
            });
            await publishSharedProject(id, buffer, this.props.projectTitle);
            if (typeof window !== 'undefined') {
                ...
                try {
                    ...
                    // Clipboard may be blocked; the URL is still in the address bar.
                }
            }
            this.setState({
                isShared: true,
                ...
                <div className={styles.mainMenu}>
                    ...
                    <div className={classNames(styles.menuBarItem)}>
                        <ShareButton
                            ...
                            isShared={this.props.isShared || this.state.isShared}
...
const mapDispatchToProps = (dispatch, ownProps) => ({
    ...
    onShowSharingAlert: () => dispatch(showStandardAlert('sharing')),
    onShowShareSuccessAlert: () => showAlertWithTimeout(dispatch, 'shareSuccess'),
    onShowShareErrorAlert: () => dispatch(showStandardAlert('shareError'))
});
