...
class RenderWebGL extends EventEmitter {
     ...
     * @private
     */
    static _getContext (canvas, extraAttribs) {
        const contextAttribs = Object.assign(
     ...
     * @param {object} [glAttribs] Optional WebGL context attributes. The unified
     *     2D overlay pass uses `{alpha: true}` so the 3D world can show through.
     * @class
     * @listens RenderWebGL#event:NativeSizeChanged
     */
    constructor (canvas, xLeft, xRight, yBottom, yTop, glAttribs) {
        super();

        /** @type {WebGLRenderingContext} */
        const gl = this._gl = RenderWebGL._getContext(canvas, glAttribs);
        if (!gl) {
        ...
        /**
         * When true, the 2D pass is an overlay on a 3D world: the backdrop is
         * skipped and the canvas is cleared transparent.
         * @type {boolean}
         */
    ...
    draw () {
        ...
        if (this.overlayMode && this._layerGroups && this._groupOrdering) {
            const bgIndex = this._groupOrdering.indexOf('background');
            const nextName = bgIndex >= 0 ? this._groupOrdering[bgIndex + 1] : null;
            if (nextName && this._layerGroups[nextName]) {
                drawList = this._drawList.slice(this._layerGroups[nextName].drawListOffset);
            }
        }
